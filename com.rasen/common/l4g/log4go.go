...
func (log Logger) intLogf(lvl Level, format string, args ...interface{}) {
	...
	src := ""
	if ok {
		fileArr := strings.SplitAfter(file,"src/")
		funcArr := strings.Split(runtime.FuncForPC(pc).Name(),".")
		src = fmt.Sprintf("%s - %s : %d",fileArr[len(fileArr)-1], funcArr[len(funcArr)-1], lineno)
...
func (log Logger) intLogc(lvl Level, closure func() string) {
	...
	pc, file, lineno, ok := runtime.Caller(2)
		src := ""
	if ok {
		fileArr := strings.Split(file,"src/")
		funcArr := strings.Split(runtime.FuncForPC(pc).Name(),".")
		src = fmt.Sprintf("%s - %s : %d",fileArr[len(fileArr)-1], funcArr[len(funcArr)-1], lineno)
	}
